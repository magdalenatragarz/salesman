...
salesman_algorithm::salesman_algorithm(std::shared_ptr<evolution::map> map, int max_iterations_without_change)
    : map(map), population(), max_iterations_without_change(max_iterations_without_change) {
...
void salesman_algorithm::generate_population(){
    ...
    auto population_factory = std::make_shared<evolution::population_factory>(genetic_material);
    auto solutions_population = population_factory->generate_population();

    for (auto& solution : solutions_population)
        population.push_back(std::make_shared<evolution::path>(solution, map));
}
...
void salesman_algorithm::generate_crossover_solution() {
    ...
    population.push_back(std::make_shared<evolution::path>(offspring_solution, map));
}
...
std::shared_ptr<evolution::path> salesman_algorithm::get_best_solution() const {
